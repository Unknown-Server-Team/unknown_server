...
import { RoleRecord, AuthenticatedRequest, PermissionRecord } from '../types';
...
class RoleManager {
    ...
    private async _initializeRoleHierarchy(): Promise<void> {
        try {
            ...
            // Build hierarchy map
            hierarchyData.forEach((entry: any) => {
    ...
    createRoleMiddleware(roleNames: string | string[]) {
    ...
    async createRole(name: string, description: string, permissions: number[] = [], parentRoleId?: number): Promise<number> {
        try {
            ...
            LogManager.info('Role created', { name, roleId });
            return roleId;
        } catch (error) {
    ...
    async getUserWithRolesAndPermissions(userId: number): Promise<UserRoleData> {
        ...
        try {
            ...
            return {
                roles,
                permissions,
                hasPermission: (permissionName: string) => permissions.some((p: PermissionRecord) => p.name === permissionName),
                hasRole: (roleName: string) => roles.some((r: RoleRecord) => r.name === roleName)
            };
        } catch (error) {
    ...
    private async _getAllParentRoleIds(roleId: number, visited = new Set<number>()): Promise<Set<number>> {
    ...
    createRoleAndPermissionMiddleware(roleNames: string | string[], requiredPermissions: string[] = [], requireAll: boolean = false) {
        return async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
            try {
                if (!req.user) {
                    return res.status(401).json({ error: 'Authentication required' });
    ...
    private async _trackRoleChange(userId: number, roleId: number, action: 'assign' | 'remove', adminId?: number): Promise<void> {
        try {
            // If AuthAnalytics is available, log the event
            const { authAnalytics } = await import('./AuthAnalytics');
